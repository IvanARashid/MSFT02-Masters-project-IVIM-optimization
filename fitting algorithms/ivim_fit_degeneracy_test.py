...
class IvimModelBiExp(ReconstModel):
    ...
    def __init__(self, gtab, f_fix=0.1, bounds=None, initial_guess=None, rescale_units=False):
    ...
    def fit(self, data):
        ...
        # Make fit attempts and assess the best fit based on sum of square diff
        attempts = 3
        sum_of_square_differences = np.zeros((attempts))
        param_estimates = np.zeros((attempts, 3))
        for i in range(attempts):
            # Randomize initial guess
            rng = uniform.rvs(size=3)
            p0 = (1*rng[0], 100e-3*rng[1], 4e-3*rng[2])
        
            # Perform the fit
            param_estimates[i,:], pcov = curve_fit(self.ivim_model_with_penalty, \
                self.bvals, data, p0=p0, maxfev=1000000)
            
            # Get the sum of square difference
            sum_of_square_differences[i] = self.sum_of_square_difference(param_estimates[i], data)
            
        # Determine the best guess as the one with the lowest sq distance
        best_idx = np.argmin(sum_of_square_differences)
        popt = param_estimates[best_idx]
        ...
        result = np.array([popt[0], self.f_fix, popt[1], popt[2]])
        result[0] *= data_max

        return IvimFit(self, result)
    
    def ivim_model(self, b, S0, D_star, D):
        return S0*(self.f_fix*np.exp(-b*D_star) + (1-self.f_fix)*np.exp(-b*D))
        

    def ivim_model_with_penalty(self, b, S0, D_star, D):
        if D >= D_star:
            penalty = 1e6
        else:
            penalty = 0
        return S0*(self.f_fix*np.exp(-b*D_star) + (1-self.f_fix)*np.exp(-b*D)) + penalty
    
    def sum_of_square_difference(self, param_estimates, data):
        fit_data = self.ivim_model(self.bvals, *param_estimates)
        sum_of_square_difference = np.sum(np.square(fit_data - data))
        return sum_of_square_difference
