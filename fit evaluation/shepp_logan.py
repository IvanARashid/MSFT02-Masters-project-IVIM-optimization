...
def ivim_shepp_logan(N, E=None, B0=3, T2star=False, zlims=(-1, 1)):
...
def mr_ellipsoid_parameters():
    ...
    """
    E = np.zeros((15, 11))
    # [:, [x, y, z, a, b, c, theta, S0, f, D*, D]]
    E[0, :] = [
        0, 0, 0, .72, .95, .93, 0,
        *params['background-biggest']]
    E[1, :] = [
        0, 0, 0, .69, .92, .9, 0,
        *params['background-2nd-biggest']]
    E[2, :] = [
        0, -.0184, 0, .6624, .874, .88, 0,
        *params['background-3rd-biggest']]
    E[3, :] = [
        0, -.0184, 0, .6524, .864, .87, 0,
        *params['background-4th-biggest']]
    E[4, :] = [
        -.22, 0, -.25, .41, .16, .21, np.deg2rad(-72),
        *params['two-sides']]
    E[5, :] = [
        .22, 0, -.25, .31, .11, .22, np.deg2rad(72),
        *params['two-sides']]
    E[6, :] = [
        0, .35, -.25, .21, .25, .35, 0,
        *params['middle-big']]
    E[7, :] = [
        0, .1, -.25, .046, .046, .046, 0,
        *params['tumor-upper']]
    E[8, :] = [
        -.08, -.605, -.25, .046, .023, .02, 0,
        *params['tumor-lower-right']]
    E[9, :] = [
        .06, -.605, -.25, .046, .023, .02, np.deg2rad(-90),
        *params['tumor-lower-left']]
    E[10, :] = [
        0, -.1, -.25, .046, .046, .046, 0,
        *params['tumor-middle']]
    E[11, :] = [
        0, -.605, -.25, .023, .023, .023, 0,
        *params['tumor-lower-middle']]
    E[12, :] = [
        .06, -.105, .0625, .056, .04, .1, np.deg2rad(-90),
        *params['tumor-middle']]
    E[13, :] = [
        0, .1, .625, .056, .056, .1, 0,
        *params['two-sides']]
    E[14, :] = [
        .56, -.4, -.25, .2, .03, .1, np.deg2rad(70),
        *params['peripheral-side']]
    """
...
def _mr_relaxation_parameters():
    ...
    params = dict()
    params['two-sides'] = [1000, 0.05, 0.005, 0.0007]
    params['peripheral-side'] = [1000, 0.5, 0.05, 0.003]
    params['middle-big'] = [1000, 0.2, 0.02, 0.003]
    params['tumor-upper'] = [1000, 0.1, 0.01, 0.0005]
    params['tumor-lower-middle'] = [1000, 0.2, 0.02, 0.0005]
    params['tumor-lower-right'] = [1000, 0.05, 0.005, 0.001]
    params['tumor-lower-left'] = [1000, 0.1, 0.01, 0.0015]
    params['tumor-middle'] = [1000, 0.3, 0.03, 0.002]
    params['background-4th-biggest'] = [1000, 0, 0, 0]
    params['background-3rd-biggest'] = [1000, 0, 0, 0]
    params['background-2nd-biggest'] = [1000, 0, 0, 0]
    params['background-biggest'] = [1000, 0, 0, 0.001]
    return params